async function carregarSaida() {
            ...
            <form id="form-romaneio">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
                    <div>
                        ...
                        <div id="romaneio-validacao" class="validacao-romaneio"></div>
                    </div>
                    <div>
                ...
                <div class="botoes-acao">
                    ...
                    <button type="button" onclick="abrirModalConfirmacaoSaida()" class="btn-registrar">✅ Confirmar Saída</button>
                </div>
            </form>
        </div>
        <div id="mensagem"></div>
    ...
    // Verificar se o número do romaneio já existe
    const inputNumero = document.getElementById('romaneio-numero');
    inputNumero.addEventListener('blur', async () => {
        ...
        if (numero) {
            ...
            const divValidacao = document.getElementById('romaneio-validacao');
            if (existente) {
                divValidacao.innerHTML = '<span style="color: #dc3545;">⚠️ Este número de romaneio já foi utilizado!</span>';
            } else {
...
function limparFormularioSaida() {
    if (confirm('Tem certeza que deseja limpar todos os campos?')) {
        ...
        adicionarItemRomaneio();
        document.getElementById('romaneio-validacao').innerHTML = '';
    }
}

function abrirModalConfirmacaoSaida() {
    // Coletar resumo dos itens para mostrar na confirmação
    const numeroRomaneio = document.getElementById('romaneio-numero').value;
    const motorista = document.getElementById('romaneio-motorista').value;
    const itensDiv = document.querySelectorAll('.item-linha');
    let totalItens = 0;
    ...
    if (!motorista) {
        showMessage('Digite o nome do motorista!', 'error');
        return;
    }
    
    const modal = document.createElement('div');
    modal.id = 'modal-confirmacao-saida';
    ...
    modal.innerHTML = `
        <div class="modal-confirmacao-content">
            ...
            <div class="modal-confirmacao-body">
                <p><strong>Número do Romaneio:</strong> ${numeroRomaneio}</p>
                ...
                <p style="color: #666;">Confirma o registro desta saída?</p>
                <p style="color: #dc3545; font-size: 12px;">⚠️ Esta ação irá dar baixa no estoque!</p>
            </div>
            <div class="modal-confirmacao-footer">
                <button onclick="fecharModalConfirmacaoSaida()" class="btn-cancelar-modal">Cancelar</button>
                <button onclick="confirmarRegistroSaida()" class="btn-confirmar-modal">Confirmar</button>
            </div>
        </div>
    `;
...
async function confirmarRegistroSaida() {
...
async function salvarRomaneio() {
    ...
    // Verificar se o romaneio já existe
    const { data: existente } = await window.supabaseClient
        .from('romaneios')
        .select('id')
    ...
    if (existente) {
        showMessage(`❌ Romaneio número ${numero_romaneio} já existe! Não é possível registrar duplicado.`, 'error');
        return;
    }
    ...
    for (const item of document.querySelectorAll('.item-linha')) {
        ...
        if (!produto || quantidade > produto.estoque_atual) {
            showMessage(`Estoque insuficiente para ${produtoId}! Disponível: ${produto?.estoque_atual || 0}`, 'error');
            return;
        }
