...
async function salvarNotaFiscal() {
    ...
    try {
        ...
        // Dentro da função salvarNotaFiscal, após salvar os itens da nota
// Substitua a criação do lote por esta versão:
...
for (const item of itens) {
    ...
    // Criar lote no estoque FIFO (VERIFICAR SE JÁ EXISTE)
    const { data: loteExistente } = await window.supabaseClient
        .from('lotes_estoque')
        .select('id')
        .eq('produto_id', item.produto_id)
        .eq('lote', item.lote || `LOTE-${Date.now()}`)
        .maybeSingle();
    
    if (!loteExistente) {
        const { error: loteError } = await window.supabaseClient
            .from('lotes_estoque')
            .insert([{
                produto_id: item.produto_id,
        ...
        // Se o lote já existe, adicionar a quantidade
        ...
        const novaQuantidade = (lote.quantidade_atual || 0) + item.quantidade;
        await window.supabaseClient
            .from('lotes_estoque')
