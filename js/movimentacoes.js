...
async function gerarRelatorioHTML() {
    try {
        ...
        // Buscar números dos documentos para cada movimentação
        ...
        for (const m of movimentacoes || []) {
            ...
            if (m.documento_tipo === 'nota_fiscal') {
                ...
                const { data: romaneio } = await window.supabaseClient
                    ...
                    .eq('id', m.documento_id)
                    .maybeSingle();
                if (romaneio && romaneio.numero_romaneio) {
            ...
            movimentacoesEnriquecidas.push({ 
                ...m, 
        ...
        const totalEntradas = movimentacoes?.filter(m => m.tipo === 'entrada').reduce((sum, m) => sum + (m.quantidade || 0), 0) || 0;
        const totalSaidas = movimentacoes?.filter(m => m.tipo === 'saida').reduce((sum, m) => sum + (m.quantidade || 0), 0) || 0;
        const totalInventario = movimentacoes?.filter(m => m.tipo === 'inventario').reduce((sum, m) => sum + (m.quantidade || 0), 0) || 0;
...
<body>
    <div class="relatorio">
        <div class="header">
            ...
            <div>Gerado em: ${formatDateTime(dataAtual)}</div>
        </div>
    ...
    <button class="btn-print" onclick="window.print()">🖨️ Imprimir / Salvar PDF</button>
        ...
        if (novaJanela) {
            ...
            showMessage('❌ O navegador bloqueou a janela. Permita pop-ups para este site.', 'error');
        }
