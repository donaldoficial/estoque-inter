async function carregarConsultar() {
    ...
    main.innerHTML = `
        <style>
            ...
            .items-preview th, .items-preview td {
                padding: 8px;
            }
            .btn-gerar-relatorio {
                margin-top: 15px;
        ...
        <div class="consulta-container">
            ...
            <div class="tipo-consulta">
                ...
                <div class="consulta-box">
                    ...
                    <div class="input-group">
                        ...
                        <button onclick="consultarRomaneio()" class="btn-romaneio">
                            🔍 Consultar
                        </button>
                    </div>
                ...
                <div class="consulta-box">
                    ...
                    <div class="input-group">
                        ...
                        <button onclick="consultarNotaFiscal()" class="btn-nota">
                            🔍 Consultar
                        </button>
                    </div>
            ...
            <div class="card-consulta">
                ...
                <div id="documentos-recentes" style="overflow-x: auto;">
                    Carregando...
                </div>
            </div>
        </div>
...
async function carregarDocumentosRecentes() {
    try {
        ...
        // Romaneios recentes
        html += '<div>';
        html += '<h4 style="margin-bottom: 15px; color: #667eea;">📦 Últimos Romaneios</h4>';
        if (romaneios && romaneios.length > 0) {
            html += `<div class="table-wrapper">`;
            html += `<table class="table">`;
            html += `<thead><tr><th>Número</th><th>Motorista</th><th>Data</th><th>Valor</th><th>Ação</th></tr></thead><tbody>`;
            romaneios.forEach(r => {
                html += `<tr>
        ...
        // Notas recentes
        html += '<div>';
        html += '<h4 style="margin-bottom: 15px; color: #28a745;">📄 Últimas Notas Fiscais</h4>';
        if (notas && notas.length > 0) {
            html += `<div class="table-wrapper">`;
            html += `<table class="table">`;
            html += `<thead><tr><th>Número</th><th>Transportadora</th><th>Data</th><th>Valor</th><th>Ação</th></tr></thead><tbody>`;
            notas.forEach(n => {
                html += `<tr>
        ...
        document.getElementById('documentos-recentes').innerHTML = '<p>Erro ao carregar documentos recentes.</p>';
    }
}
    ...
    try {
        ...
        if (!romaneio) {
            resultadoDiv.innerHTML = `
                <div class="resultado-erro" style="padding: 15px; border-radius: 8px;">
                    ❌ Romaneio ${numero} não encontrado!
                </div>
            `;
            resultadoDiv.className = 'resultado-consulta show';
            return;
        }
        ...
        // Mostrar preview
        resultadoDiv.innerHTML = `
            <div class="resultado-sucesso" style="padding: 15px; border-radius: 8px;">
                <h4 style="margin-bottom: 10px;">✅ Romaneio Encontrado!</h4>
                <div class="info-documento">
                    <p><strong>Número:</strong> ${romaneio.numero_romaneio}</p>
                    <p><strong>Motorista:</strong> ${romaneio.motorista || '-'}</p>
                    <p><strong>Placa:</strong> ${romaneio.placa_veiculo || '-'}</p>
                    <p><strong>Destinatário:</strong> ${romaneio.destinatario || '-'}</p>
                ...
                <button onclick="gerarRelatorioRomaneioCompleto('${romaneio.id}')" class="btn-gerar-relatorio" style="margin-top: 15px;">
...
async function consultarNotaFiscalNumero(numero) {
    ...
    try {
        ...
        resultadoDiv.innerHTML = `
            <div class="resultado-sucesso" style="padding: 15px; border-radius: 8px;">
                <h4 style="margin-bottom: 10px;">✅ Nota Fiscal Encontrada!</h4>
                <div class="info-documento">
                ...
                <button onclick="gerarRelatorioNotaFiscalCompleto('${nota.id}')" class="btn-gerar-relatorio" style="margin-top: 15px;">
                    📄 Gerar Relatório Completo (PDF)
                </button>
            </div>
        `;
...
// Função para gerar relatório completo do Romaneio
async function gerarRelatorioRomaneioCompleto(romaneioId) {
    try {
        ...
        await gerarRelatorioRomaneio(romaneio, itens);
    } catch (error) {
        showMessage('Erro ao gerar relatório: ' + error.message, 'error');
    }
}

// Função para gerar relatório completo da Nota Fiscal
async function gerarRelatorioNotaFiscalCompleto(notaId) {
    try {
        ...
        await gerarRelatorioNotaFiscal(nota, itens);
    } catch (error) {
        showMessage('Erro ao gerar relatório: ' + error.message, 'error');
    }
}

// Gerar relatório do Romaneio (chamado do saida.js)
async function gerarRelatorioRomaneio(romaneio, itens) {
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header .numero { font-size: 18px; opacity: 0.9; margin-top: 10px; }
        .info-section {
            padding: 30px;
            background: #f8f9fa;
...
<body>
    <div class="documento">
        <div class="header">
            ...
            <div class="numero">Nº ${romaneio.numero_romaneio}</div>
        ...
        <div class="info-section">
            <div class="info-grid">
                <div>
                    <div class="info-item">
                        ...
                        <div class="info-value">${romaneio.motorista || '-'}</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">PLACA DO VEÍCULO</div>
                        <div class="info-value">${romaneio.placa_veiculo || '-'}</div>
                    </div>
                </div>
                <div>
                    <div class="info-item">
                        <div class="info-label">DESTINATÁRIO</div>
                        <div class="info-value">${romaneio.destinatario || '-'}</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">DATA DE SAÍDA</div>
                        <div class="info-value">${formatDate(romaneio.data_saida)}</div>
                    </div>
                </div>
            </div>
            ${romaneio.observacao ? `
            <div style="margin-top: 15px;">
                ...
                <div class="info-value">${romaneio.observacao}</div>
            </div>
            ` : ''}
        </div>
        ...
        <div class="total-geral">
            TOTAL DO ROMANEIO: ${formatMoney(romaneio.valor_total)}
        </div>
        
        <div class="footer">
            <p>Documento emitido pelo Sistema de Controle de Estoque</p>
...
// Gerar relatório da Nota Fiscal
async function gerarRelatorioNotaFiscal(nota, itens) {
    const dataAtual = new Date();
...
<body>
    <div class="documento">
        <div class="header">
            ...
            <div class="numero">Nº ${nota.numero_nota}</div>
            <div>Emitido em: ${formatDateTime(dataAtual)}</div>
        </div>
        ...
        <div class="info-section">
            <div class="info-grid">
                ...
                <div>
                    <div class="info-item">
                        ...
                        <div class="info-value">${nota.transportadora || '-'}</div>
                    </div>
                </div>
            </div>
            ${nota.observacao ? `
            <div style="margin-top: 15px;">
                <div class="info-label">OBSERVAÇÃO</div>
                <div class="info-value">${nota.observacao}</div>
            </div>
            ` : ''}
        </div>
        
        <div class="tabela-container">
            <h3 style="margin-bottom: 15px;">ITENS DA NOTA FISCAL</h3>
            <table>
                <thead>
                    <tr>
                        <th>Código</th>
        ...
        <div class="total-geral">
            TOTAL DA NOTA: ${formatMoney(nota.valor_total)}
        </div>
