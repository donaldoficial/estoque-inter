...
function carregarLogin() {
    ...
    if (typeof adicionarCaptcha === 'function') {
        setTimeout(() => adicionarCaptcha(), 100);
    }
...
async function fazerLogin() {
    ...
    if (typeof verificarCaptcha === 'function') {
        ...
        if (!captchaValido) {
            mostrarMensagemLogin('❌ Marque o "Não sou um robô" para continuar!', 'error');
            if (typeof resetarCaptcha === 'function') {
                setTimeout(() => resetarCaptcha(), 100);
            }
            return;
        }
    }
